...
class ManifoldGraphDataset(Dataset):
    def __init__(self, full_graphs, subgraph_k, degree_features, subsample_pctg=0.5):
    ...
    def prepare_data(self):
        ...
        for full_graph in self.full_graphs:
            ...
            for idx in tqdm(indices, desc=f'Processing new graph'):
                ...
                subgraph = Data(x=full_graph.x[subgraph_nodes], edge_index=subgraph_edges, edge_attr=edge_attrs, y=full_graph.y[idx])
                if self.degree_features:
